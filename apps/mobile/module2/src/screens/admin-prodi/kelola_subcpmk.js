...
const INITIAL_DATA = [
  { id: '1', kode: 'SCPL-01', mk: 'Analisis & Perancangan Sistem', desc: 'Mampu membuat DFD dan ERD', bobot: '30%' },
  { id: '2', kode: 'SCPL-02', mk: 'Manajemen Basis Data Bisnis', desc: 'Mampu merancang struktur tabel relasional', bobot: '40%' },
  { id: '3', kode: 'SCPL-03', mk: 'Arsitektur Enterprise untuk Bisnis', desc: 'Mampu merancang kerangka kerja TOGAF', bobot: '50%' },
];

const MK_OPTIONS = [
  'Analisis & Perancangan Sistem',
  'Manajemen Basis Data Bisnis',
  'Arsitektur Enterprise untuk Bisnis',
...
export default function KelolaSubCpmkScreen({ navigation }) {
  ...
  const [selectedMk, setSelectedMk] = useState('Pilih Mata Kuliah');
  ...
  const handleAdd = () => {
    if (!kode || !desc || !bobot || selectedMk === 'Pilih Mata Kuliah') {
      setAlertConfig({ 
        visible: true, 
        type: 'error', 
        title: 'Lengkapi Data! 🚨', 
        message: 'Pastikan semua data lengkap sebelum menyimpan Sub-CPMK baru.' 
      });
      return;
    }
    ...
    setTimeout(() => {
      setAlertConfig({ 
        ...
        type: 'success', 
        title: 'Berhasil!', 
        message: 'Data Sub-CPMK berhasil disimpan ke dalam sistem.' 
      });
    }, 300);
  };
  ...
  const closeModal = () => {
    ...
    setSelectedMk('Pilih Mata Kuliah'); setMkOpen(false);
  };
  ...
  const renderItem = ({ item }) => (
    ...
    >
      ...
      <View style={styles.cardContent}>
        ...
        {/* Teks dibatasi maksimal 2 baris */}
        <Text style={styles.cardSubtitle} numberOfLines={2}>
          Bobot: {item.bobot} | {item.desc}
        </Text>
      </View>
    </TouchableOpacity>
  );
    ...
    <ImageBackground source={require('../../../assets/uinsa2.jpeg')} style={styles.container} imageStyle={{ opacity: 0.15 }}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTextWrap}>
          ...
          <Text style={styles.headerSubtitle}>Indikator Penilaian Program Studi Sistem Informasi</Text>
        </View>
      </View>
      ...
      <TouchableOpacity style={styles.fab} onPress={() => setModalVisible(true)} activeOpacity={0.8}><Ionicons name="add" size={28} color="#212121" /></TouchableOpacity>

      <Modal visible={modalVisible} animationType="slide" transparent onRequestClose={closeModal}>
        <View style={styles.modalOverlay}>
          <TouchableWithoutFeedback onPress={() => { Keyboard.dismiss(); closeModal(); }}><View style={StyleSheet.absoluteFillObject} /></TouchableWithoutFeedback>
          
          <View style={styles.modalContentLucu}>
            <View style={styles.modalHandle} />
            <Text style={styles.modalTitleLucu}>Tambah Sub-CPMK</Text>
            
            <View style={styles.inputContainer}>
              <Ionicons name="barcode-outline" size={20} color={PRIMARY_BLUE} style={styles.inputIcon} />
              <TextInput style={styles.inputLucu} placeholder="Kode Sub-CPMK (Contoh: SCPL-05)" placeholderTextColor="#94A3B8" value={kode} onChangeText={setKode} />
            </View>
            ...
            <View style={{ marginBottom: 12 }}>
              <View style={styles.inputContainerDropdown}>
                ...
                <TouchableOpacity style={styles.dropdownTrigger} onPress={() => { Keyboard.dismiss(); setMkOpen(true); }}>
                  <Text style={[styles.dropdownValue, selectedMk === 'Pilih Mata Kuliah' && {color: '#94A3B8'}]} numberOfLines={1}>{selectedMk}</Text>
            ...
            <View style={styles.inputContainer}>
              ...
              <Text style={{ fontFamily: 'Urbanist', fontSize: 16, color: '#64748B', marginLeft: 10 }}>%</Text>
            </View>
        ...
        <TouchableOpacity style={styles.alertOverlay} activeOpacity={1} onPress={() => setAlertConfig({...alertConfig, visible: false})}>
          <TouchableWithoutFeedback onPress={() => {}}>
            <View style={styles.alertBox}>
              ...
              {/* FIX: Tombol Alert jadi Fit Text */}
              <TouchableOpacity 
                ...
                <>
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 20 }}>
                    <View style={[styles.cardAvatar, { marginRight: 15 }]}>
                      <Text style={styles.avatarText}>{selectedSubCpmk.kode.slice(-2)}</Text>
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.cardTitle}>{selectedSubCpmk.kode}</Text>
                      <Text style={{ fontFamily: 'Urbanist-Regular', fontSize: 13, color: '#64748B' }}>
                        {selectedSubCpmk.mk}
                      </Text>
                    </View>
                  </View>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
                    <Text style={{ fontFamily: 'Urbanist-Bold', fontSize: 14, color: '#212121' }}>
                      Bobot Penilaian: 
                    </Text>
                    <View style={{ backgroundColor: '#e0f2f1', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, marginLeft: 10 }}>
                      <Text style={{ fontFamily: 'Urbanist-Bold', fontSize: 13, color: '#00796b' }}>
                        {selectedSubCpmk.bobot}
                      </Text>
                    </View>
                  </View>
                  
                  <Text style={{ fontFamily: 'Urbanist-Bold', fontSize: 14, color: '#212121', marginBottom: 8, marginTop: 5 }}>
                    Deskripsi Indikator:
                  </Text>
                  <Text style={{ fontFamily: 'Urbanist-Regular', fontSize: 14, color: '#64748B', lineHeight: 22, marginBottom: 25 }}>
                    {selectedSubCpmk.desc}
                  </Text>
...
const styles = StyleSheet.create({
  ...
  cardSubtitle: { fontFamily: 'Urbanist-Regular', fontSize: 12, color: '#64748B', paddingRight: 10 },
