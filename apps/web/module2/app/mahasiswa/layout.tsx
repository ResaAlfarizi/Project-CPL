...
export default function MahasiswaLayout({ children }: { children: React.ReactNode }) {
  ...
  if (user.role && user.role.toLowerCase() !== 'mahasiswa') return null;
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-700">
          <div className="flex items-center gap-3 px-3 py-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white text-sm font-medium truncate">mahasiswa</p>
        ...
        <header className="bg-white h-16 flex items-center px-4 lg:px-6 gap-4 border-b border-gray-200">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">
              {navItems.find(item => item.href === pathname)?.label === 'Dashboard' 
                ? 'Data program studi dan Capaian Pembelajaran Lulusan (hanya baca)'
                : 'Sistem CPL - Modul 2'}
            </p>
          </div>
        </header>
